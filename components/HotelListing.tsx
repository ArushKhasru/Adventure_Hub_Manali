...
const essentials = [
  { icon: Icons.BedDouble, title: "Comfort-first rooms", copy: "Well-kept rooms, cosy bedding, and the essentials for an easy mountain night." },
  { icon: Icons.Utensils, title: "Good food nearby", copy: "Choose stays close to cafess, local kitchens, and unhurried breakfast spots." },
  { icon: Icons.Trees, title: "A better location", copy: "Pick between the river, Old Manali lanes, Vashisht calm, or Solang access." },
];
      ...
      <div className="mx-auto max-w-[1270px]">
        ...
        <div className="mt-8 grid gap-x-8 gap-y-12 md:grid-cols-2">
          {stays.map((stay, index) => <article key={stay.name} className={`group grid gap-5 ${index % 2 === 1 ? "md:mt-16" : ""}`}>
            <div className="relative aspect-[16/10] overflow-hidden rounded-[1.5rem] bg-[#d9d7cd] shadow-[0_1rem_2.5rem_rgba(28,69,53,.12)]"><Image src={stay.image} alt={stay.alt} fill sizes="(max-width: 767px) 100vw, 50vw" className="object-cover transition duration-700 group-hover:scale-105" /><div className="absolute inset-x-4 top-4 flex items-center justify-between"><span className="rounded-full bg-[#f5f1e8]/90 px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[.15em] text-[#174b3c] backdrop-blur">{stay.tag}</span><span className="grid size-9 place-items-center rounded-full bg-[#174b3c] text-white"><Icons.ArrowUpRight className="size-4" /></span></div></div>
