...
export default function HotelHeroExplorer() {
  return (
    <section className="relative isolate overflow-hidden bg-[#f2eee4] px-5 pb-8 pt-24 sm:px-8 sm:pt-28 lg:px-12 lg:pb-12 lg:pt-20">
      ...
      <div className="mx-auto grid max-w-[1370px] overflow-hidden rounded-[2rem] bg-[#174b3c] text-white shadow-[0_1.75rem_5rem_rgba(22,54,43,.2)] lg:grid-cols-[minmax(0,1fr)_minmax(26rem,.9fr)]">
        <div className="relative flex min-h-[38rem] flex-col justify-between overflow-hidden px-6 py-8 sm:px-10 sm:py-10 lg:min-h-[42rem] lg:px-14 lg:py-14">
          ...
          <div className="relative">
            <p className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[.2em] text-[#cbe9d9]"><Icons.MapPin className="size-4 text-[#edc879]" /> Manali, Himachal Pradesh</p>
          ...
          <div className="relative flex flex-wrap items-end justify-between gap-6 pt-10">
            <Link href="#stays" className="inline-flex min-h-12 items-center gap-3 rounded-full bg-[#f0c975] px-5 text-sm font-extrabold text-[#174b3c] no-underline transition hover:-translate-y-0.5 hover:bg-[#ffe0a1]">Explore stays <Icons.ArrowDownRight className="size-4" /></Link>
            <div className="border-l border-white/20 pl-4 text-sm text-white/75"><span className="block font-[family-name:var(--font-display)] text-3xl font-extrabold leading-none text-white">4.8</span><span className="mt-1 flex items-center gap-1"><Icons.Star className="size-3 fill-[#f0c975] text-[#f0c975]" /> guest-loved stays</span></div>
          </div>
        </div>
